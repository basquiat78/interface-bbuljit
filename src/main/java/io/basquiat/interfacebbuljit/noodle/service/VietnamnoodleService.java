...
/**
 * 베트남 쌀국수 레시피를 통해서 면을 만들고 반환을 한다.
 *
 */
@Service("vietnamnoodle")
@RequiredArgsConstructor
public class VietnamnoodleService implements MenuFetcher {

    @Override
    public RecipeFetcher<Topping, ResponseResult<NoodleDto>> cooking() {
        return topping -> responseResult(NoodleDto.builder()
                                                  .noodleName(NoodleType.VIETNAMNOODLE.getName())
                                                  .topping(topping)
                                                  .build()
                                        );
    }
